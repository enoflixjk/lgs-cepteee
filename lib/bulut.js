...
// EKLEME TALİMATI
//
// Bu dosyanın içeriğini kopyala, kendi lib/bulut.js dosyanın SONUNA
// yapıştır (var olan hiçbir şeyi silme, sadece ekle). Dosyanın
// başında zaten `import { supabase } from './supabase';` gibi bir
// satır olduğunu varsayıyorum — eğer değişkenin adı farklıysa
// (örn. `supabaseClient`) aşağıdaki `supabase` kelimesini onunla
// değiştir.
// ============================================================

// Bu cihazdaki kullanıcının "şu an aktif" zaman damgasını günceller.
// Hata olursa sessizce yutulur — bu özellik olmazsa olmaz değil,
// asıl uygulamayı hiçbir zaman bozmamalı.
export async function sonAktifligimiGuncelle() {
  try {
    await supabase.rpc('son_aktifligimi_guncelle');
  } catch (e) {
    // sessizce geç
  }
}

// Verilen grup kodundaki, kendisi hariç, son 10 dakikada aktif olan
// arkadaş sayısını döner. Hata ya da grup yoksa 0 döner.
export async function grupAktifSayisiniGetir(grupKodu) {
  if (!grupKodu) return 0;
  try {
    const { data, error } = await supabase.rpc('grup_aktif_sayisi', { p_grup_kodu: grupKodu });
    if (error) return 0;
