...
// ============================================================
const SEVIYE_TANIM = [
  { ad: 'Çaylak',  minXp: 0,    ikon: Sprout,        r: '#38EF7D', rk: '#11998E' },
  { ad: 'Öğrenci', minXp: 100,  ikon: BookOpenCheck, r: '#00C6FF', rk: '#0072FF' },
  { ad: 'Azimli',  minXp: 300,  ikon: Zap,           r: '#FFB020', rk: '#E5860B' },
  { ad: 'Uzman',   minXp: 600,  ikon: Gem,           r: '#D04ED6', rk: '#834D9B' },
  { ad: 'Usta',    minXp: 1200, ikon: Swords,        r: '#FF5A5F', rk: '#B31217' },
  { ad: 'Efsane',  minXp: 2500, ikon: Crown,         r: '#FFD200', rk: '#F7971E' },
];

export const xpdenSeviye = (xp) => {
  const liste = SEVIYE_TANIM.map(s => ({
    ad: s.ad, minXp: s.minXp, ikon: s.ikon, renk: s.r, renkKoyu: s.rk,
  }));
  let g = liste[0];
...
export function TemaSaglayici({ koyu, setKoyu, altBosluk, children }) {
  const deger = useMemo(() => {
    ...
    return {
      ...
      DERSLER: derslerAl(),
      seviyeHesapla: (xp) => xpdenSeviye(xp),
    };
...
export function useTema() {
  ...
  if (!t) {
    ...
    return {
      ...
      DERSLER: derslerAl(), seviyeHesapla: (x) => xpdenSeviye(x),
    };
  }
  return t;
}
