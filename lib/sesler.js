...
const SES_DOSYA = {
  ...
  kartcevir: (() => { try { return require('../assets/sesler/kartcevir.m4a'); } catch (e) { return null; } })(),
};
...
const SES_DUZEYI = { dogru: 0.7, yanlis: 0.7, seviye: 0.7, kartcevir: 0.35 };
...
async function seslerOnYukle() {
  ...
  await Promise.all(Object.keys(SES_DOSYA).map(async (isim) => {
    ...
    try {
      const { sound } = await Audio.Sound.createAsync(SES_DOSYA[isim], { volume: SES_DUZEYI[isim] ?? 0.7 });
