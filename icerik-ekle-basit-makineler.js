// ============================================================
// BASİT MAKİNELER KARTLARINI EKLE
//
// data/cards.js dosyasının sonuna, basit-makineler.js içindeki 38
// kartı ekler. Yedek alır, id çakışması varsa hiçbir şey
// yazmadan durur.
//
// Kullanım:
//   1) basit-makineler.js dosyasını proje köküne koy
//   2) node icerik-ekle-basit-makineler.js
// ============================================================
...
const YEDEK = path.join(__dirname, 'data', 'cards.basitmakineler-oncesi.js');
const YENI_KARTLAR = path.join(__dirname, 'basit-makineler.js');
...
function calistir() {
  ...
  if (!fs.existsSync(YENI_KARTLAR)) {
    console.error('✗ basit-makineler.js bulunamadı. Onu da proje köküne koy.');
    process.exit(1);
  }
  ...
  kaynakMetin = kaynakMetin.replace('export const BASIT_MAKINELER_KARTLARI', 'const YENI');
  kaynakMetin += '\nmodule.exports = YENI;\n';
  ...
  if (!fs.existsSync(YEDEK)) {
    ...
    console.log('✓ Yedek alındı: data/cards.basitmakineler-oncesi.js');
  }
  ...
  console.log('  cp data/cards.basitmakineler-oncesi.js data/cards.js');
}
